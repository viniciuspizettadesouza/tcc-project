...
| Amostra histórica de 10.000 da PoC 1 | amostra usada nas Figuras 20–22, sem população inicial, ordem ou semente publicadas | amostra determinística de 10.000 após filtros documentados, semente 42 | igualdade das linhas não pode ser testada | seleção histórica não foi preservada no PDF nem no notebook recuperado | irreproduzível |
| Execução histórica final da PoC 2 | resultados narrados para modelos, score e Figuras 23–29 | execução nova integral, com ambiente e outputs registrados | equivalência exata da execução não pode ser demonstrada | células finais não têm outputs preservados; versões, defaults e estado de execução históricos são desconhecidos | irreproduzível |

## Cobertura final

Após a validação integral, os 29 itens comparados estão classificados: 11 `reproduzido`, 9 `parcial`, 5 `divergente`, 2 `conflitante` e 2 `irreproduzível`. Não há item pendente ou `não executado` na tabela. `Parcial` permanece quando a tese não publica precisão suficiente para uma comparação quantitativa ou quando a reconstrução deliberadamente usa agregados seguros; isso não indica ausência de execução.
